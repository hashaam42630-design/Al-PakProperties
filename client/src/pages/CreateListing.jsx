...
export default function CreateListing() {
  ...
  const handleImageSubmit = async () => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      try {
        ...
        setImageUploadError('Image upload failed (Base64 conversion error)');
        setUploading(false);
      }
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/listing/create', {
        ...
        body: JSON.stringify({
          ...formData,
          userRef: currentUser._id, // This links the listing to your real account
        }),
      });
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
      <h1 className='text-3xl font-semibold text-center my-7 text-slate-700'>Create a Listing</h1>
